...
public class Expression extends Literal {
    ...
    public static List<ICalculable> parseWrappedList(List<ICalculable> postfix) {
        for (int i = 0; i < postfix.size(); i++) {
            ...
            if (calculable.getType() == ICalculableType.LITERAL) {
    ...
    public Object execute() throws ExpressionException, IncomparableTypeException {
        ...
        for (ICalculable literal : expression) {
            if (literal.getType() == ICalculableType.FLAG)
                inFuncDec = true;
            else if (literal instanceof Do)
                inLoopDec = true;
            else if (literal instanceof ForLoop)
                inLoopDec = false;
            else if (literal.getType() == ICalculableType.CONDITIONAL_PLACEHOLDER)
                inCondDec = true;
    ...
    private boolean validate() {
        ...
        for (ICalculable literal : expression) {
            ...
            if (literal.getType() == ICalculableType.OPERATOR && inFunc == 0)
                stackSize -= ((IOperator) literal).getNumOperands();
            else if (literal.getType() == ICalculableType.FUNCTION || literal.getType() == ICalculableType.USER_FUNCTION) {
                inFunc--;
                stackSize -= funcSize;
                funcSize = 0;
            }
            else if (literal.getType() == ICalculableType.FUNCTION_PLACEHOLDER)
                inFunc++;
            else if (literal.getType() == ICalculableType.CONDITIONAL_PLACEHOLDER || literal.getType() == ICalculableType.FLAG)
                stackSize--;
        }
